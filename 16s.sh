[TOC]
    ...
    # **每次打开Rstudio必须运行下面4行**
    ea=/mnt/e/工作/16s扩增子分析/刘永鑫博士—16s扩增子全流程/EasyAmplicon-master/EasyAmplicon-master
    wd=/mnt/e/test
    PATH=$PATH:${ea}/win
    cd ${wd}
...
## 2. 合并双端序列并按样品重命名 Merge paired reads and label samples
    ...
    # 以KO1单样品合并为例
    vsearch --fastq_mergepairs seq/KO1_1.fq.gz \
      --reverse seq/KO1_2.fq.gz \
      --fastqout temp/KO1.merged.fq \
      --relabel KO1.
...
## 2. PICRUSt功能预测
    ...
    #按`功能级别`分类汇总, -c输出KEGG_Pathways，分1-3级
